...
const ROLE_OPTIONS = [
    { id: 'user', value: 'user', title: 'User', description: 'Base role with standard access', icon: User, badge: 'Base Role' },
    { id: 'vendor', value: 'vendor', title: 'Vendor', description: 'Can manage products and inventory', icon: Store, badge: '+ User' },
    { id: 'admin', value: 'admin', title: 'Admin', description: 'Full administrative privileges', icon: UserCog, badge: '+ Vendor + User' },
    { id: 'super_admin', value: 'super_admin', title: 'Super Admin', description: 'Complete system control', icon: ShieldCheck, badge: '+ Admin + Vendor + User' },
];

const AssignRoleModal = ({ isOpen, onClose, user, onConfirm, isLoading }) => {
    const { canAssignRole, isSelf, isSuperAdmin } = useUserPermissions(user);

    const { control, watch, handleSubmit, reset } = useForm({
        defaultValues: { role: user?.roles?.[0] ?? 'user' },
    });

    const roleValue = watch('role');

    useEffect(() => {
        if (user) reset({ role: user.roles?.[0] ?? 'user' });
    }, [user, reset]);

    const currentRole = user?.roles?.[0];
    const newRoles = ROLE_HIERARCHY[roleValue] ?? ['user'];
    const isRoleChanged = roleValue !== currentRole;
    const primaryRole = roleValue;
    const secondaryRoles = newRoles.slice(1);

    const onSubmit = (data) =>
        onConfirm({ role: data.role, roles: ROLE_HIERARCHY[data.role] });
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-xs text-blue-700 dark:text-blue-300 leading-relaxed">
                                    Each role automatically includes all lower-level roles. For example, assigning
                                    &quot;Admin&quot; also grants &quot;Vendor&quot; and &quot;User&quot; permissions.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-100 dark:bg-green-950/50 border border-green-300 dark:border-green-700 rounded-lg">
                                ...
                                <span className="text-xs text-green-700 dark:text-green-300 ml-1">(Primary)</span>
                            </div>
